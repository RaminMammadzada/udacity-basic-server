...
(async () => {
  ...
  app.post( "/cars/", ( req: Request, res: Response ) => {
    ...
    // check if id is uneque or not
    const carWithTheId = cars.filter( (car) => car.id === parseInt(id) )
    if (carWithTheId.length !== 0) {
      return res.status(409).send(`the car with id ${id} is already existed, it cannot be dublicated!`)
    }
